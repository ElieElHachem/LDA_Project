...
def number_of_phenotypes_to_test_with_vector(dimension, number_of_subsets_to_select):
    ...
    phenotypes_randomly_picked = random.sample(list_of_vector_of_probability, number_of_subsets_to_select)
    return list_of_vector_of_probability , phenotypes_randomly_picked
...
def test_blob(number_of_patient,n_pheno,n_dim,theta,number_of_cell, patient_frame_to_store,value, return_combination):
    ...
    for group_patient_phenotype , list_patient in enumerate(list_of_array_patients):
        exp_dis = make_exponential_distribution(n_dim, theta)
...
def test_extreme_porba(dimension, value_for_middle):

    array = np.full(shape=2**dimension, fill_value=value_for_middle/2**dimension)
    array[0] = array[0] + (1-value_for_middle)
    ...
    for configuration in np.arange(0,(1-value_for_middle),value_for_middle/2**dimension):
        new_arr = array.copy()
        new_arr_2 = array_2.copy()
...
def test_extreme_porba_subseted(dimension, value_for_middle,n_cat,n_sample):
    ...
    proba_to_test = {'proba_to_test' : [], 'euc_dist' : [] }
    ...
    for configuration in np.arange(0,(1-value_for_middle),value_for_middle/2**dimension):
        ...
        proba_to_test['proba_to_test'].append([new_arr,new_arr_2])

    for value in proba_to_test['proba_to_test']:
        euc_dist = euclidean(*value)
        proba_to_test['euc_dist'].append(euc_dist)

    dataframe = pd.DataFrame(proba_to_test)

    categories = pd.cut(dataframe['euc_dist'], n_cat)
    dataframe['category'] = categories
    values = dataframe.groupby('category')['proba_to_test'].apply(lambda s: s.sample(n_sample, random_state=42))
